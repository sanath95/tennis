...
class DataSet():
	...
	def __init__(self, cnn_model, 
		...
		self.sequence_path = os.path.join('E:/opencv/lrcnn/data', 'sequences')
	...
	def get_data():
		...
		with open(os.path.join('E:/opencv/lrcnn/data', 'data_file.csv'), 'r') as fin:
	...
	def get_frames_for_sample(self, sample):
		...
		if 'backhand2h' in sample[2]:
			sample[1]='backhand2hands'

		if 'bslice' in sample[2]:
			sample[1]='backhand_slice'

		if 'bvolley' in sample[2]:
			sample[1]='backhand_volley'

		if 'serflat' in sample[2]:
			sample[1]='flat_service'

		if 'foreflat' in sample[2]:
			sample[1]='forehand_flat'

		if 'foreopen' in sample[2]:
			sample[1]='forehand_openstands'

		if 'fslice' in sample[2]:
			sample[1]='forehand_slice'

		if 'fvolley' in sample[2]:
			sample[1]='forehand_volley'

		if 'serkick' in sample[2]:
			sample[1]='kick_service'

		if 'serslice' in sample[2]:
			sample[1]='slice_service'

		path = os.path.join('E:/opencv/thetis/VIDEO_RGB', sample[1], sample[2] + '.avi')
	...
	def extract_seq_features(self, sample):
		...
		path = os.path.join('E:/opencv/lrcnn/data', 'sequences', sample[1], sample[2] + '-' + str(self.seq_length) + \
		'-features')
